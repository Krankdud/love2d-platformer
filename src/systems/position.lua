...
--- Initializes a PositionSystem.
-- System that processes entities with a position component. It only processes
-- the position on one of the axes, so two systems are required to process both axes.
-- @param axis The axis to process ("x" or "y")
function PositionSystem:initialize(axis)
	self.axis = axis
end

--- Processes entities with a position component
...
function PositionSystem:process(e, dt)
	if self.axis == "x" then
		self:_process_axis(e.position, "x", "prevX", "subpixelX")
	else
		self:_process_axis(e.position, "y", "prevY", "subpixelY")
	end
end

-- Private function to handle updating an axis
function PositionSystem:_process_axis(position, axis, prevAxis, subpixelAxis)
