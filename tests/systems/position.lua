    ...
    describe("when given the x axis", function()
        ...
        describe("processes an entity", function()
            it("sets entity.position.x to an integer", function()
                ...
                ps:process(entity, 0)
                assert.is.equal(entity.position.x, 2)
            end)
    
            it("sets entity.position.subpixelX", function()
                entity.position.x = 2.5
                ps:process(entity, 0)
                assert.is.equal(entity.position.subpixelX, 0.5)
            end)
            ...
            it("sets prevX", function()
                entity.position.x = 2
                ps:process(entity, 0)
                assert.is.equal(entity.position.prevX, 2)
            end)

            it("doesn't modify y", function()
                entity.position.y = 2.5
                ps:process(entity, 0)
                assert.is.equal(entity.position.y, 2.5)
            end)
        end)
    end)
    ...
    describe("when given the y axis", function()
        ...
        describe("processes an entity", function()
            it("sets entity.position.y to an integer", function()
                entity.position.y = 2.5
                ps:process(entity, 0)
                assert.is.equal(entity.position.y, 2)
            end)
    
            it("sets entity.position.subpixelY", function()
                entity.position.y = 2.5
                ps:process(entity, 0)
                assert.is.equal(entity.position.subpixelY, 0.5)
            end)
            ...
            it("sets prevY", function()
                ...
                entity.position.y = 2
                ps:process(entity, 0)
                assert.is.equal(entity.position.prevY, 2)
            end)

            it("doesn't modify x", function()
                entity.position.x = 2.5
                ps:process(entity, 0)
                assert.is.equal(entity.position.x, 2.5)
            end)
        end)
    end)
end)
